                    ...
                    <c:otherwise>
                        <li>
                            <a href="userLogout" id="logout">
                                <span class="fa fa-unlock-alt" aria-hidden="true"></span><i>${user.userName}</i> : Sign Out </a>
                        </li>
                    </c:otherwise>
                </c:choose>
            </ul>
    ...
    <div class="container">
        ...
        <c:if test="${cart!=null}">
            <div class="checkout-right">
                ...
                <h4>您的购物车包含：
                    <span>${number} 个商品</span>
                </h4>
                <div class="table-responsive">
                    <table class="timetable_sub">
                        <thead>
                        <tr>
                            <th width="40px"><input type="checkbox" id="all" onclick="checkAllCart(this.checked)"/></th>
                            <th>编号</th>
                            <th>商品名称</th>
                            <th>商品单价</th>
                            <th>数量</th>
                        ...
                        <tbody class="cart1">
                        <c:if test="${cart!=null}">
                            <c:forEach items="${cart}" var="cart" varStatus="count">
                                <tr>
                                    <th><input type="checkbox" name="one" onclick="checkOneCart()" value="${cart.key}"/>
                                    </th>
                                    <th>&nbsp;${count.count}</th>
                                    <th>&nbsp;${cart.value.freshGoods.goodName}</th>
                                    <th>&nbsp;$${cart.value.freshGoods.price*cart.value.freshGoods.discount/100}</th>
                                    <th>&nbsp;${cart.value.quantity}</th>
                                    <th>&nbsp;
                                        <c:if test="${cart.value.freshGoods.preference==1}">肉食</c:if>
                                        <c:if test="${cart.value.freshGoods.preference==2}">素食</c:if>
                                        <c:if test="${cart.value.freshGoods.preference==0}">其他</c:if>
                                    </th>
                                    <th>
                                        &nbsp;$${cart.value.freshGoods.price*cart.value.freshGoods.discount/100*cart.value.quantity}</th>
                                </tr>
                    ...
                    <form action="payment" method="post" class="creditly-card-form agileinfo_form">
                        <div class="creditly-wrapper wthree, w3_agileits_wrapper">
                            <div class="information-wrapper">
                                ...
                                <c:forEach items="${cart}" var="cart" varStatus="count">
                                    <input type="hidden" name="fdid" value="${cart.key}"/>
                                    <input type="hidden" name="discount"
                                           value="${cart.value.freshGoods.price*cart.value.freshGoods.discount/100}"/>
                                    <input type="hidden" name="price"
                                           value="${cart.value.freshGoods.price*cart.value.freshGoods.discount/100*cart.value.quantity}"/>
                                </c:forEach>
                    ...
                    <div class="checkout-right-basket">
                        <form action="payment" id="myForm" method="post">
                            <c:forEach items="${cart}" var="cart" varStatus="count">
                                ...
                                <%--                        <a href="" onclick="submit()">Make a Payment--%>
                                <%--                            <span class="fa fa-hand-o-right" aria-hidden="true"></span>--%>
                                <%--                        </a>--%>
                            <button class="submit check_out">Make a Payment<span class="fa fa-hand-o-right"
...
<script>
    $.get("/", function (data) {
        for (var key in data) {
            ...
            var img = $("<a href='single2.jsp'><img src=" + imgs[0] + "'img/.jpg' alt='' class='img-responsive'></a>");
            td2.append(img);
